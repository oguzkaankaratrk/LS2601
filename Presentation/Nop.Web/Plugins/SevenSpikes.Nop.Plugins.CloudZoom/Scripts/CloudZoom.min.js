...
$(document).ready(function () {
    ...
    // Slick tamamen yüklendiğinde çalışması için interval
    var waitForArrows = setInterval(function () {

        var $prev = $(".picture-thumbs .slick-prev");
        var $next = $(".picture-thumbs .slick-next");

        if ($prev.length && $next.length) {

            // TAŞIMA
            $(".picture-wrapper").append($prev);
            $(".picture-wrapper").append($next);

            // Stil: büyük resim üzerine gelsin
            $prev.addClass("zoom-arrows");
            $next.addClass("zoom-arrows");

            clearInterval(waitForArrows);
        }

    }, 100); // 100ms kontrol eder
});
